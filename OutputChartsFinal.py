...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        text ="Please enter the Period, the Worst Case Execution time, and the Actual Execution time for each Task.", font = SMALLFONT)
        instuctions_P_label = ttk.Label(self, 
        text ="You can use preselected P-State values and/or enter your own.", font = SMALLFONT)
        instuctions_num_label = ttk.Label(self, 
        text ="This simulator accepts a maximum of 5 Tasks and 8 P-states.", font = SMALLFONT)
        ...
        instuctions_label.grid(row = 2, column = 0, padx = 10, pady = 5, sticky="w")
        instuctions_P_label.grid(row = 3, column = 0, padx = 10, pady = 5, sticky="w")
        instuctions_num_label.grid(row = 5, column = 0, padx = 10, pady = 5, sticky="w")
        ...
        button1.grid(row = 6, column = 0, padx = 10, pady = 10, sticky="se")
...
# Create a Master Frame on left of Page 1 to contain all Task Entry frames and/or widgets
        ...
        frame_left.grid(column=0,row=0,rowspan=16,padx=5,pady=5,sticky="news")
        ...
        frame_add_remove_T.grid(column=0,row=16,columnspan=4, pady=10, padx=20,sticky="nsew")
...
# Create a Master Frame on right of Page 1 to contain all P-State Entry frames and/or widgets
        ...
        frame_right.grid(row=0, rowspan=16,column=1,columnspan=4, padx=5,pady=5,sticky="news")
        ...
        frame_add_remove_p.grid(row=16, column=0)
        
        
        
# Button Frame Creation #########################################################################################
# Create Frame on Bottom for "Return to instructions", "Reset all values to zero" and " Schedule Feasibility" buttons
        Button_frame = Frame(self,highlightbackground="grey", highlightthickness=1)
        Button_frame.grid(column=0, row=16, columnspan=8, pady=5, padx=5,sticky="news")
...
################################################################################################################## 
        ...
        def  clr_all (): # clear all numerical values (both for the p-state and the tasks)
            
            for widgets in frame1.winfo_children():
                    widgets.destroy() 
            #This destroys everything in the frame - we must "re-build "      
            #create Task 1 labels        
            Task_1_label= tk.Label( frame1, text="Task 1")
            Period_label= tk.Label( frame1, text="Period", font = SMALLERFONT)
            Worst_Case_label= tk.Label( frame1, text="Worst Case", font = SMALLERFONT)
            Actual_Execution_label= tk.Label(frame1, text="Actual", font = SMALLERFONT)
        
            # Place Default Task 1 Text Labels
            Task_1_label.grid(row=2, column=0, sticky="nsew")
            Period_label.grid(row=1, column=1, sticky="nsew")
            Worst_Case_label.grid(row=1, column=2, sticky="nsew")
            Actual_Execution_label.grid(row=1, column=3, sticky="nsew")  
            
            # Redefine entry boxes variables so they can be accessed afetr
            global T1PE
            global T1WC
            global T1BC
            
            # Reset task 1 entry boxes to zero 
            T1PE = tk.Entry(frame1)
            T1WC = tk.Entry(frame1)
            T1BC = tk.Entry(frame1)
            ...
            # destroy the entries for tasks 2-5
            for widgets in frame_T2E.winfo_children():
                    widgets.destroy() 
            for widgets in frame_T3E.winfo_children():
                    widgets.destroy()
            ...
            global count_T
            count_T = 1 #reset task count to 1

            # Remove checkmark from pre selcted P-states     
            var1.set('0')        
            var2.set('0')
            var3.set('0')
            # Remove all custom P-states       
            ...
            #null the output of the P-state
            global Pstate_output
            Pstate_output=[]   
            #reset p-state counter to 0     
            global count_P        
            count_P = 0 
#                                             P State ADD/REMOVE                                                #
##################################################################################################################        
        ...
        ADD_P = ttk.Button(frame_add_remove_p, text='(+) ADD P-STATE', command=add_p).grid(row = 8, column = 1, padx = 10, pady = 10,sticky='s')
        # Create Remove task button and place
        REMOVE_P = ttk.Button(frame_add_remove_p, text='(-) REMOVE P-STATE', command=remove_p).grid(row = 8, column = 2, padx = 10, pady = 10,sticky='s')
...
#####################                          PAGE 1  BUTTONS                      #############################
        ...
        button1 = ttk.Button(Button_frame, text ="<<< Return to Instructions",
                            command = lambda : controller.show_frame(StartPage))
        button1.grid(row = 0, column=0,padx=5,pady=5, sticky="w")

        # Create "Check Schedule feasibility" button and place
        button2 = ttk.Button(Button_frame, text ="Check Schedule Feasibility >>>", command = read_data_T )
        button2.grid(row = 0, column=8,padx=5, pady=5, sticky="e")
        
        # Create Reset all values to 0
        CLR_ALL = ttk.Button(Button_frame, text =">>> Reset all Values to 0 <<<", command = clr_all )
        CLR_ALL.grid(row = 0, column = 4, padx=104, pady=5,sticky="news")

        # Create Task Button and place
        ADD_P = ttk.Button(frame_add_remove_T, text='(+) ADD TASK', command=add).grid(row = 14, column = 1, padx = 10, pady = 10, sticky='w')

        # Create Remove task button and place
        REMOVE_P = ttk.Button(frame_add_remove_T, text='(-) REMOVE TASK', command=remove).grid(row=14, column = 2, padx = 10, pady = 10,sticky='e')
